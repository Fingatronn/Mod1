...
public class PumpUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		double y1 = 0;
		double z1 = 0;
		if (world.getBlockState(new BlockPos(x + x1, y - 1, z)).canOcclude() == false && (world.getBlockState(new BlockPos(x + x1, y - 1, z))).getBlock() == ProbModBlocks.OIL.get()) {
		...
		if (world.getBlockState(new BlockPos(x, y - 1, z + z1)).canOcclude() == false && (world.getBlockState(new BlockPos(x, y - 1, z + x1))).getBlock() == ProbModBlocks.OIL.get()) {
